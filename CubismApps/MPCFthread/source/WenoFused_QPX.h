...
struct Weno_QPX_fused
{
	...
	void weno_minus_plus_fused_opt1(const vector4double a, const vector4double b, const vector4double c, const vector4double d, const vector4double e, const vector4double f, Real * minus, Real * plus) //133 FLOP
	{
		...
		const vector4double m_alpha0 = myreciprocal<preclevel>(vec_mul(m_is0plus, m_is0plus));		
		const vector4double m_alpha1 = mydivision<preclevel>(vec_splats(6), vec_mul(m_is1plus, m_is1plus));
		const vector4double m_alpha2 = mydivision<preclevel>(vec_splats(3), vec_mul(m_is2plus, m_is2plus));
		...
		const vector4double p_alpha0 = myreciprocal<preclevel>(vec_mul(p_is0plus, p_is0plus));		
		const vector4double p_alpha1 = mydivision<preclevel>(vec_splats(6), vec_mul(p_is1plus, p_is1plus));
		const vector4double p_alpha2 = mydivision<preclevel>(vec_splats(3), vec_mul(p_is2plus, p_is2plus));
		...
		
		{
			...
			vec_sta(vec_mul(minus_tmp, F_2_6), 0L, minus);
			vec_sta(vec_mul(plus_tmp, F_2_6), 0L, plus);
		}
		// 28
	}
	...
	void weno_minus_plus_fused_opt2(const vector4double a, const vector4double b, const vector4double c, const vector4double d, const vector4double e, const vector4double f, Real *minus, Real *plus) //127 FLOP
	{
		...
		
		{
			...
			vec_sta(vec_mul(minus_tmp, vec_mul(F_2_6, m_inv_alpha)), 0L, minus);
			vec_sta(vec_mul(plus_tmp , vec_mul(F_2_6, p_inv_alpha)), 0L, plus);
		}
	}
};
