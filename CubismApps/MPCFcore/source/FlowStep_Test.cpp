...
void FlowStep_Test::_print(Block& block)
{
	for(int iz = 0; iz<_BLOCKSIZE_; iz++)
		for(int iy = 0; iy<_BLOCKSIZE_; iy++)
			for(int ix = 0; ix<_BLOCKSIZE_; ix++)
...
void FlowStep_Test::_gold(TestLab& lab, Block& block, const Real h_gridpoint)
{
	for(int iz = 0; iz<_BLOCKSIZE_; iz++)
		for(int iy = 0; iy<_BLOCKSIZE_; iy++)
			for(int ix = 0; ix<_BLOCKSIZE_; ix++)
...
void FlowStep_Test::_compare(Block& _a, Block& _b, double accuracy, bool bAwk, string kernelname)
{
...
void FlowStep_Test_Babak::_initialize(TestLab& lab, Block& block)
{
	for(int iz = -3; iz<_BLOCKSIZE_+3; iz++)
		for(int iy = -3; iy<_BLOCKSIZE_+3; iy++)
			for(int ix = -3; ix<_BLOCKSIZE_+3; ix++)
			{
				...
				lab(ix, iy, iz).s.r = 2.0;
				lab(ix, iy, iz).s.u = 3.0;
				lab(ix, iy, iz).s.v = 4.0;
				lab(ix, iy, iz).s.w = 6.0;
				lab(ix, iy, iz).s.levelset = -1.25;
				lab(ix, iy, iz).s.s = 100.5;
			}	
	...
	for(int iz = 0; iz<_BLOCKSIZE_; iz++)
		for(int iy = 0; iy<_BLOCKSIZE_; iy++)
			for(int ix = 0; ix<_BLOCKSIZE_; ix++)
			{
				...
				block(ix, iy, iz).dsdt.r = 0.0;
				block(ix, iy, iz).dsdt.u = 0.0;
				block(ix, iy, iz).dsdt.v = 0.0;
				block(ix, iy, iz).dsdt.w = 0.0;
				block(ix, iy, iz).dsdt.s = 0.0;
				block(ix, iy, iz).dsdt.levelset = 0.0;
			}
}
