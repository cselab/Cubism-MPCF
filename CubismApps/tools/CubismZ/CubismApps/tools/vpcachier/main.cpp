...
*/
	...
	else //ok we are ready to start
	{
		...
		for(int g = mystart; g < myend; ++g)
		{
			...
			WaveletTexture3D * texture = new WaveletTexture3D;
			WaveletsOnInterval::FwtAp * const ptrtexdata = & texture->data()[0][0][0]; // float if _FLOAT_PRECISION_ is defined
			...
			//normalize the data
			{
				...
				for(int i = 0; i < _VOXELS_ * _VOXELS_ * _VOXELS_; ++i)
				{
					...
					ptrtexdata[i] = std::min(static_cast<Real>(1.), std::max(static_cast<Real>(0.), a0 + a1 * val));
