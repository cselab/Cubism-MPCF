...
if [ -z "$4" ]
then
	echo "missing argument - blocksize"
	exit
else
	bs=$4
fi
...
echo "XXXXXXXXXXXXXXXXXXX - > $method"

#RES=wavz_${DATE}.`echo ${TEST#./} | tr " " "-" `_res.txt
#RES=wavz_`echo $2 | tr " " "-" `_res.txt
#RES=`echo $2 | sed 's/\// /g' | awk '{print $NF}'`_wavz1_res.txt
#RES=`echo $2 | sed 's/\// /g' | awk '{print $NF}'`_wavz`echo $wt`.`echo $method`_res.txt
RES=`echo $2 | sed 's/\// /g' | awk '{print $NF}'`_wavz`echo $wt`.`echo $bs`_res.txt
#echo $RES
