...
namespace WaveletsOnInterval
{
	...
	struct FullTransformEngine<4, ROWSIZE, COLSIZE, SLICESIZE>
	{
        ...
        template <int BS>
        struct __attribute__((__aligned__(CUBISM_ALIGNMENT))) FullTransform
            : FullTransformEngine<BS, BS, BS, BS> {
            FwtAp data[BS][BS][BS];

            void fwt(int wtype)
            {
                FullTransformEngine<BS, BS, BS, BS>::fwt(data, wtype);
            }

            void iwt(int wtype)
            {
                FullTransformEngine<BS, BS, BS, BS>::iwt(data, wtype);
            }

            template <typename DataType, int REFBS>
            int threshold(const FwtAp eps,
                          bitset<REFBS * REFBS * REFBS> &mask_survivors,
                          DataType *const buffer_survivors)
            {
                return FullTransformEngine<BS, BS, BS, BS>::
                    template threshold<DataType, BS>(
                        eps, mask_survivors, buffer_survivors, data);
            }

            template <typename DataType>
            void load(vector<DataType> &datastream, bitset<BS * BS * BS> mask)
            {
                FullTransformEngine<BS, BS, BS, BS>::template load<DataType>(
                    datastream, mask, data);
            }
        };
}
