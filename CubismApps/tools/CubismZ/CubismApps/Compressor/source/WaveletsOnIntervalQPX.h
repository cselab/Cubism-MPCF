...
namespace WaveletsOnInterval
{
	...
	{
            template <int NH>
            struct __attribute__((__aligned__(CUBISM_ALIGNMENT)))
            TinyScratchPad {
                FwtAp c[NH][4];
                FwtAp d[NH][4];

                void _cpbk(FwtAp *src,
                           FwtAp *dst0,
                           FwtAp *dst1,
                           FwtAp *dst2,
                           FwtAp *dst3)
                {
                    for (int is = 0, id = 0; is < NH * 4; is += 16, id += 4) {
                        ...
                        vector4double d2 =
                            vec_lda(sizeof(FwtAp) * 4 * 2, src + is);
                        vector4double d3 =
                            vec_lda(sizeof(FwtAp) * 4 * 3, src + is);
                ...
                void
                _put(Real *dst, Real *src0, Real *src1, Real *src2, Real *src3)
                {
                ...
                void
                put(Real *stream0, Real *stream1, Real *stream2, Real *stream3)
                {
                    assert(NH % 4 == 0);
