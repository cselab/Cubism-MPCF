...
struct GlobalData
{
    double V_global; // has dimension
    double center[3];
    int nBlocks;

    double wallCellsVolume;
    unsigned long long int wallCells;
    ArgumentParser& parser;
    GlobalData(ArgumentParser& p) :
        V_global(0.), nBlocks(0), wallCellsVolume(0.), wallCells(0), parser(p)
    { center[0] = center[1] = center[2] = 0.; }
};

struct BlockData
{
    double cell_volume; // has dimension
...
struct SphericalObserverQoI : public ConditionalMinMaxQoI<true> // we don't care whether true/false here, as update member is overloaded below
{
    ...
    SphericalObserverQoI(const std::string name, const double value, const Real pos_[3], const double radius) :
        ConditionalMinMaxQoI<true>(name, value), radius(radius), value_min(HUGE_VAL), value_max(-HUGE_VAL), discrete_volume(0.), count(0)
    {
        pos[0] = pos_[0];
        pos[1] = pos_[1];
        pos[2] = pos_[2];
    ...
    inline void update(const double new_value, const Real new_pos[3], const double cell_volume)
    {
    ...
    inline void update(const double new_value, const double new_min, const double new_max, const double new_volume, const unsigned long long int lcount)
    {
        value += new_value;
        discrete_volume += new_volume;
