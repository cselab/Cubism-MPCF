...
class Test_ChannelFlowMPI :
    public Test_ChannelFlow<TGrid,TStepper,TSlice,TSubdomain>,
...
public:
    Test_ChannelFlowMPI(const MPI_Comm comm, ArgumentParser& P) :
        ...
        Test_ChannelFlow<TGrid,TStepper,TSlice,TSubdomain>(P),
        Test_SteadyStateMPI<TGrid,TStepper,TSlice,TSubdomain>(comm,P)
    { }
...
protected:
    ...
    virtual void _setEnvironment()
    {
        ...
        m_relax = this->parser.parseRuntime("body_force_relax").asDouble();
    }
    ...
    virtual void _setup_parameter()
    {
        ...
        Test_ChannelFlow<TGrid,TStepper,TSlice,TSubdomain>::_parameter_channel_flow();
        ...
        m_relax = this->parser("body_force_relax").asDouble(1.0);
    }

    virtual void _init()
    {
        Test_SteadyStateMPI<TGrid,TStepper,TSlice,TSubdomain>::_init();
...
double Test_ChannelFlowMPI<TGrid,TStepper,TSlice,TSubdomain>::_computeFlowRate()
{
    ...
    for (size_t l = 0; l < myloc.size(); l++)
    {
        ...
        for(int i=0; i<vInfo.size(); i++)
        {
            ...
            const int bstart = info.index[0]*TBlock::sizeX;
            const int bend = bstart + TBlock::sizeX;
            if (gx < bstart || bend <= gx) continue;

            const int ix = (gx % (this->BPDX*TBlock::sizeX)) % TBlock::sizeX;

            const double* const hy = (this->grid->getMeshMap(1)).get_grid_spacing(info.index[1]);
            const double* const hz = (this->grid->getMeshMap(2)).get_grid_spacing(info.index[2]);
            for(int iz=0; iz<TBlock::sizeZ; iz++)
                for(int iy=0; iy<TBlock::sizeY; iy++)
                    sum += b(ix,iy,iz).ru * hy[iy] * hz[iz];
        }
    }
    t1 = omp_get_wtime();
    ...
    if (this->isroot)
    {
        ...
        out.precision(12);
        out << '\t' << this->t;
        const double invLz = 1.0/(this->grid->getMeshMap(2)).extent();
        for (size_t i = 0; i < gUsum.size(); ++i)
        {
            mdot_avgOverLoc += gUsum[i]*invLz;
            out << '\t' << gUsum[i]*invLz;
        }
        out << std::endl;
        out.close();
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_ChannelFlowMPI<TGrid,TStepper,TSlice,TSubdomain>::_setBodyforce(const bool bSave)
{
    ...
    if (invocation > 0)
    {
        ...
        Simulation_Environment::vol_bodyforce[0] += m_relax*u;
    }
    else
        old = e;
    ++invocation;
