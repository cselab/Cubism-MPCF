...
namespace SinglePhaseStatistics
{
    ...
    public:
        ...
        inline void updateCell(const BlockData& data, const int ix, const int iy, const int iz, const BlockInfo& info)
        {
            ...
            enstropy.update(vol * data.kenstrophy);
            epsilon1.update(vol * data.energyDissipationRate);
            epsilon2.update(vol * data.divu*data.divu);
            epsilon3.update(vol * data.p*data.divu);
    ...
    public:
        ...
        inline void updateCell(const BlockData& data, const int ix, const int iy, const int iz, const BlockInfo& info)
        {
            ...
            IuI.update(cell_volume * data.IuI, pos, cell_volume);
            W.update(cell_volume * data.W, pos, cell_volume);
    ...
    void dumpStatistics(TGrid& grid, const int step_id, const Real t, const Real dt, ArgumentParser& parser)
    {
        ...
        if (bVort || bAll) evaluateOperator_MPI<LabMPI, OVort>(grid); // this writes into tmp.[0], tmp.[1] and tmp.[2]
        // compute div(u)
        const bool bDivu = parser("-statistics_computeDivu").asBool(false);
        if (bDivu || bAll) evaluateOperator_MPI<LabMPI, OdivU>(grid); // this writes into tmp.[3]
        // S^d : S^d
        const bool bSS = parser("-statistics_computeS:S").asBool(false);
        if (bSS || bAll) evaluateOperator_MPI<LabMPI, OSSd>(grid); // @dummy
        // |grap(p)|
