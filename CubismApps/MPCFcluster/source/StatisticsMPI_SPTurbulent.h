...
#pragma omp critical
    ...
    void dumpStatistics(TGrid& grid, std::vector<AverageQoI>& hist, const int step_id, const Real t, const Real dt, ArgumentParser& parser)
    {
        ...
        const size_t yCells = grid.getMeshMap(1).ncells();
        ...
        if (isroot)
        {
            const double xCells = grid.getMeshMap(0).ncells();
            const double zCells = grid.getMeshMap(2).ncells();
            const size_t updateCount = hist.front().updateCount; // must use hist.front(), see updateStatistics function
            const double factor = 1.0/(updateCount*xCells*zCells); // averaging factor
            ...
            out << "# Grid Spacing = " << vInfo[0].h_gridpoint << std::endl;
#else
            out << "# Grid Spacing non-uniform" << std::endl;
            ...
            {
                const double dy = grid.getMeshMap(1).cell_width(cell);
                out << ypos + 0.5*dy;
                ypos += dy;
