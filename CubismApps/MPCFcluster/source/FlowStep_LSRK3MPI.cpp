...
#ifdef _USE_HPM_
    if (LSRK3data::step_id>0)             HPM_Start("SURFTENS");
#endif
...
Real FlowStep_LSRK3MPI<GridMPI_t>::operator()(const Real max_dt, const Real current_time)
{
    ...
    if (parser("-kernels").asString("cpp")=="cpp")
        _process_LSRK3<Convection_CPP_HLLC_5eq_nonuniform, Source_CPP, Diffusion_CPP_NU, SurfaceTension_CPP_NU, Update_CPP, Update_State_CPP_5eq, InterfaceSharpening_CPP_cell_compact_5eq>(grid, dt, current_time);
