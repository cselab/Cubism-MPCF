...
class Test_SteadyStateMPI: public virtual Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>
{
public:

    Test_SteadyStateMPI(const MPI_Comm comm, ArgumentParser& P) :
        Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>(P), m_comm_world(comm)
    {
        int rank;
...
protected:
    ...
    virtual void _save()
    {
        Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::_save();
...
void Test_SteadyStateMPI<TGrid,TStepper,TSlice,TSubdomain>::_setup_parameter()
{
    ...
    if (this->parser("restart_format").asString("zbin") == "zbin") // we prioritize zbin
        this->_serializationKernel = &DumpZBin_MPI<StreamerSerialization, TGrid>;
#ifdef _USE_HDF_
    else if (this->parser("restart_format").asString("zbin") == "h5")
        this->_serializationKernel = &DumpHDF5_MPI<StreamerSerialization, DumpReal, TGrid>;
#endif
    else
    {
        if (this->isroot)
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SteadyStateMPI<TGrid,TStepper,TSlice,TSubdomain>::setup()
{
    _setup_parameter();
    ...
    this->m_nonuniform = new NonUniformScheme<typename TGrid::BlockType, WENOCoefficientGenerator, FDCoefficientGenerator>(0.0,eX,0.0,eY,0.0,eZ, NBlocksX, NBlocksY, NBlocksZ);
    ...
    this->dumper = new OutputProcessingMPI<TGrid,TSlice,TSubdomain>(this->parser, *(this->grid), this->isroot);
    assert(this->dumper != NULL);
    this->dumper->register_all(*(this->grid));

    const std::string path = this->parser("-fpath").asString(".");
    typename Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::TDeserializer _deserializer = NULL;
    if (exist_restart("zbin", path)) // we prioritize zbin (MPI topology must be the same as at serialization time)
        _deserializer = &ReadZBin_MPI<StreamerSerialization, TGrid>;
#ifdef _USE_HDF_
    else if (exist_restart("h5", path))
        _deserializer = &ReadHDF5_MPI<StreamerSerialization, DumpReal, TGrid>;
#endif
