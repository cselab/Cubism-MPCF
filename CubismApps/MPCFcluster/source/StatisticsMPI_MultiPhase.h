...
namespace MultiPhaseStatistics
{
    ...
    public:
        ...
        {
            const double vol = data.cell_volume;

            r_avg.update(vol * data.r);
            r2_avg.update(vol * data.r2);
            u_avg.update(vol * data.u);
            v_avg.update(vol * data.v);
            w_avg.update(vol * data.w);
            m_avg.update(vol * data.IuI);
            W_avg.update(vol * data.W);
            ke_avg.update(vol * data.ke);
            e_avg.update(vol * data.e);
            p_avg.update(vol * data.p);
            c_avg.update(vol * data.c);
            M_avg.update(vol * data.M);
            rInt.update(vol * data.r);
            uInt.update(vol * data.u*data.r);
            vInt.update(vol * data.v*data.r);
            wInt.update(vol * data.w*data.r);
            EInt.update(vol * data.e);
            EIntv.update(vol * data.e * data.a2);
            EkInt.update(vol * data.ke);
            thick.update(vol * data.a2*(1.0 - data.a2));
            IgA2I.update(vol * data.dummy);
            mg.update(vol * data.r2);
            p2_avg.update(vol * data.p*data.a2);
            if (info.index[2]==0 && iz==0)
                pw_avg.update(vol * data.p);
        }
        ...
        {
            const double factor = 1.0 / data.V_global;
            ...
            if (data.wallCellsVolume > 0)
                pw_avg.value /= data.wallCellsVolume;
        }

        inline void writeHeader(std::ofstream& stream) const
        {
            for (int i = 0; i < m_pelements.size(); ++i)
                stream << " " << m_pelements[i]->name;
        }

        inline void writeInfo(std::ofstream& stream) const
        {
            for (int i = 0; i < m_pelements.size(); ++i)
                stream << m_pelements[i]->name << std::endl;
        }

        inline void writeData(std::ofstream& stream) const
        {
            for (int i = 0; i < m_pelements.size(); ++i)
    ...
    public:
        ...
        {
            for (int i = 0; i < m_pelements.size(); ++i)
                m_pelements[i]->update(rhs.m_pelements[i]->value, rhs.m_pelements[i]->value_min, rhs.m_pelements[i]->value_max, rhs.m_pelements[i]->discrete_volume, rhs.m_pelements[i]->count);
        }
        ...
        inline void updateCell(const BlockData& data, const int ix, const int iy, const int iz, const BlockInfo& info)
        {
            Real pos[3];
            info.pos(pos, ix, iy, iz);
            const double cell_volume = data.cell_volume;

            r.update(cell_volume * data.r,               pos, cell_volume);
            r2.update(cell_volume * data.r2,             pos, cell_volume);
            u.update(cell_volume * data.u,               pos, cell_volume);
            v.update(cell_volume * data.v,               pos, cell_volume);
            w.update(cell_volume * data.w,               pos, cell_volume);
            m.update(cell_volume * data.IuI,             pos, cell_volume);
            W.update(cell_volume * data.W,               pos, cell_volume);
            ke.update(cell_volume * data.ke,             pos, cell_volume);
            e.update(cell_volume * data.e,               pos, cell_volume);
            p.update(cell_volume * data.p,               pos, cell_volume);
            c.update(cell_volume * data.c,               pos, cell_volume);
            M.update(cell_volume * data.M,               pos, cell_volume);
            a2.update(cell_volume * data.a2,             pos, cell_volume);
            p2.update(cell_volume * data.p*data.a2,      pos, cell_volume);
            divu.update(cell_volume * data.divu,         pos, cell_volume);
            K.update(cell_volume * data.K,               pos, cell_volume);
            Kdivu.update(cell_volume * data.K*data.divu, pos, cell_volume);
            gradp.update(cell_volume * data.gradp,       pos, cell_volume);
        }
        ...
        inline void reduce(MPI_Comm comm, MPI_Op operation)
        {
            ...
            std::vector<double> values(m_pelements.size());
            std::vector<double> volumes(m_pelements.size());
            ...
            for (int j = 0; j < m_pelements.size(); j++)
            {
                ...
                volumes[j]    = m_pelements[j]->discrete_volume;
#ifdef _STATISTICS_OBSERVER_MINMAX_
            ...
            MPI_Reduce(isroot ? MPI_IN_PLACE : &volumes[0], &volumes[0], volumes.size(), MPI_DOUBLE, operation, 0, comm);
#ifdef _STATISTICS_OBSERVER_MINMAX_
            ...
            for (int j = 0; j < m_pelements.size(); j++)
            {
                ...
                m_pelements[j]->discrete_volume  = volumes[j];
#ifdef _STATISTICS_OBSERVER_MINMAX_
        ...
        {
            for (int j = 0; j < m_pelements.size(); j++)
                if (m_pelements[j]->discrete_volume != 0)
                    m_pelements[j]->value /= m_pelements[j]->discrete_volume;
        }
        ...
        inline void postMPI(const GlobalData& data, const SphericalObserverSet& other)
        {
            for (int j = 0; j < m_pelements.size(); j++)
            {
                ...
                const double c1   = m_pelements[j]->discrete_volume;
                const double sum0 = other.m_pelements[j]->value;
                const double c0   = other.m_pelements[j]->discrete_volume;
                m_pelements[j]->value = ((c1-c0) > 0. ? (sum1-sum0)/(c1-c0) : 0.0);
                m_pelements[j]->discrete_volume = (c1-c0) > 0. ? c1-c0 : 0.0;
            }
        }
        ...
        inline void writeInfo(std::ofstream& stream) const
        {
            for (int i = 0; i < m_pelements.size(); ++i)
            {
                ...
                stream << ", " << m_pelements[i]->pos[1];
                stream << ", " << m_pelements[i]->pos[2] << ")" << std::endl;
                stream << '\t' << "Radius = " << m_pelements[i]->radius << std::endl;
                stream << '\t' << "Volume = " << m_pelements[i]->discrete_volume << std::endl;
                stream << '\t' << "Cells  = " << m_pelements[i]->count << std::endl;
            }
        }
    ...
    // ============================================================================
    // helper
    std::vector<SphericalObserverSet> getSensors(ArgumentParser& parser, const GlobalData& data)
    {
        const int sensors = parser("-sensors").asInt(0);
        std::vector<SphericalObserverSet> ret;
    ...
    std::vector<SphericalObserverSet> getShells(ArgumentParser& parser, const GlobalData& data)
    {
        ...
        if (n_shells > 0)
        {
            ...
            // specify the center of the shells only if all coordinates are
            // specified
            const bool bcx = parser.exist("shell_center_x");
            const bool bcy = parser.exist("shell_center_y");
            const bool bcz = parser.exist("shell_center_z");
            Real center[3];
            if (bcx && bcy && bcz)
            {
                center[0] = parser("shell_center_x").asDouble();
                center[1] = parser("shell_center_y").asDouble();
                center[2] = parser("shell_center_z").asDouble();
            }
            else
            {
                // choose domain center for shell center
                for (int i = 0; i < 3; ++i)
    ...
    void dumpStatistics(TGrid& grid, const int step_id, const Real t, const Real dt, ArgumentParser& parser)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            gdata.V_global *= Simulation_Environment::extents[i];
            gdata.nBlocks *= grid.getBlocksPerDimension(i);
            gdata.center[i] = 0.5*Simulation_Environment::extents[i];
        }
        ...
        // some work here too
        // TODO: [fabianw@mavt.ethz.ch; Fri May 12 2017 03:18:06 PM (-0700)]
        // These FD operators can not be used for nonuniform meshes yet!!

        // compute |grad(alpha2)|
        const bool bGradA = parser("-statistics_computeGradA").asBool(false);
        if (bGradA || bAll) evaluateOperator_MPI<LabMPI, OIgradA2I>(grid);
        // compute vorticity
        const bool bVort = parser("-statistics_computeVorticity").asBool(false);
        if (bVort || bAll) evaluateOperator_MPI<LabMPI, OVort>(grid);
        // compute div(u)
        const bool bDivu = parser("-statistics_computeDivu").asBool(false);
        if (bDivu || bAll) evaluateOperator_MPI<LabMPI, OdivU>(grid);
        // |grap(p)|
...
#pragma omp for schedule(dynamic,1)
            for(int i=0; i<(int)g_vInfo.size(); i++)
            {
                ...
                for(int iz=0; iz<TBlock::sizeZ; iz++)
                    for(int iy=0; iy<TBlock::sizeY; iy++)
                        for(int ix=0; ix<TBlock::sizeX; ix++)
                        {
                            ...
                            bdata.cell_volume = 1.0;
                            const int IJK[3] = {ix, iy, iz};
                            for (int k = 0; k < 3; ++k)
                            {
#ifdef _NONUNIFORM_BLOCK_
                                const double* const h = info.ptr_grid_spacing[k];
                                bdata.cell_volume *= h[ IJK[k] ];
#else
                                bdata.cell_volume *= info.uniform_grid_spacing[k];
#endif /* _NONUNIFORM_BLOCK_ */
                            }

                            bdata.V2 += bdata.cell_volume * bdata.a2;
                            if (phase_01) bdata.V2_01 += bdata.cell_volume;
                            if (phase_05) bdata.V2_05 += bdata.cell_volume;
                            if (phase_10) bdata.V2_10 += bdata.cell_volume;
                            if (phase_50) bdata.V2_50 += bdata.cell_volume;
                            if (phase_90) bdata.V2_90 += bdata.cell_volume;
                            if (phase_95) bdata.V2_95 += bdata.cell_volume;
                            if (phase_99) bdata.V2_99 += bdata.cell_volume;

                            if (info.index[2]==0 && iz==0)
                            {
                                ++bdata.wallCells;
                                bdata.wallCellsVolume += bdata.cell_volume;
                            }
                ...
                myWallCellsVolume += bdata.wallCellsVolume;
...
#pragma omp critical
            {
                gdata.wallCellsVolume += myWallCellsVolume;
        ...
        if (isroot)
        {
            if (bInfo)
            {
                ...
                info << "STATISTICS INFO:" << std::endl;
                info << "================" << std::endl;
                info << "Integral Quantities:" << std::endl;
                integralSet.writeInfo(info);
                info << std::endl << "Global Minimum/Maximum:" << std::endl;
                globalMinSet.writeInfo(info);
                globalMaxSet.writeInfo(info);
                info << std::endl << "Liquid Minimum/Maximum:" << std::endl;
                liquidMinSet.writeInfo(info);
                liquidMaxSet.writeInfo(info);
                info << std::endl << "Gas Minimum/Maximum:" << std::endl;
                gasMinSet.writeInfo(info);
                gasMaxSet.writeInfo(info);
                info << std::endl << "Spherical Observer:" << std::endl;
                info << "Sensors:" << std::endl;
                for (int i = 0; i < sensors.size(); i++)
                {
                ...
                info << std::endl << "Observer for Shells:" << std::endl;
                for (int i = 0; i < shells.size(); i++)
                {
                ...
                std::cout << "Wrote statistics.info" << std::endl;
