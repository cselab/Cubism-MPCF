...
print 'usage: python benchmark-silvio.py {icc|gcc} <desired-time-in-minutes> <footprint-per-thread-in-megabytes> [strong]'
...
if re.search("gcc", stage) != None:
	run.run(runargs, tdesired_seconds, "54.4", "20.", footprint_megabytes, common, "./silvio-exec-gcc461/", nodedata+"-gcc461", "", threads, threads_per_node, desired_bpd)

#for the moment not supported	
if re.search("icc", stage) != None:
	run.run(runargs, tdesired_seconds, "54.4", "20.", footprint_megabytes, common + " source /opt/intel/composerxe/bin/compilervars.sh intel64; ", "./silvio-exec-icc12/", nodedata+"-icc12", "", threads, threads_per_node, desired_bpd)
