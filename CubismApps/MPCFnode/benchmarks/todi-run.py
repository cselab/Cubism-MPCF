...
print 'usage: python tody-run.py gcc <desired-time-in-minutes> <footprint-per-thread-in-megabytes>'
...
#:/users/petrosk/numactl-2.0.7/lib64/
#export PATH=/users/petrosk/numactl-2.0.7/:$PATH ; \
...
if re.search("gcc", stage) != None:
	run.run(runargs, tdesired_seconds, "33.6", "60.", footprint_megabytes, common + " module load gcc/4.6.1; ", "./todi-exec-gcc461/", nodedata+"-gcc461", "", threads, threads_per_node)
