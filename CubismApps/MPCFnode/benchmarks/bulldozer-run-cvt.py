...
print 'usage: python bulldozer-run-cvt.py {icc|gcc} <desired-time-in-minutes> <path-to-IC>'
...
threads = [8, 4, 2, 1];
runargs = " -ncores 4 -dumpperiod 100000000 -saveperiod 100000000 -ascii 0 -awk 0 -bubx 0.2 -buby 0.5 -bubz 0 -cfl 0.6 -g1 1.4 -g2 1.667 -gr 0 \
...
if re.search("gcc", stage) != None:
	runcvt.run(runargs, tdesired_seconds, "57.6", "20.", common, "./bulldozer-exec-cvt-gcc461/", nodedata+"-gcc461", "", threads, threads_per_node)

#for the moment not supported	
if re.search("icc", stage) != None:
	runcvt.run(runargs, tdesired_seconds, "57.6", "20.", common + " source /opt/intel/composerxe/bin/compilervars.sh intel64; ", "./bulldozer-exec-icc12/", nodedata+"-icc12", "", threads, threads_per_node)
