...
public:
	...
	{	
		...
		if (info.index[2]==0)			bc.template applyBC_reflecting<2,0>();//bc.template applyBC_absorbing_better_faces<2,0>();
		if (info.index[2]==this->NZ-1)	bc.template applyBC_reflecting<2,1>();//bc.template applyBC_absorbing_better_faces<2,1>();
