...
public:
    ...
    void _apply_bc(const BlockInfo& info, const Real t=0)
    {
        ...
        const double pressure = CloudData::p1 + CloudDataAcoustic::p_amplitude*std::sin(2.0*M_PI*CloudDataAcoustic::frequency*(t*std::sqrt(10.0)*1.0e-6)); // convert t from cubism-mpcf units to seconds
...
public:
    ...
    void _apply_bc(const BlockInfo& info, const Real t=0)
    {
        ...
        // {
        //     BGC::BoundaryBlock& myBblock = BGC::bgblock_list_dir0_side1[info.index[1]%BGC::BPDY+BGC::BPDY*(info.index[2]%BGC::BPDZ)];
        //     // BlockType& b = *(BlockType*)info.ptrBlock;

        //     // for (int iz = 0; iz < BlockType::sizeZ; ++iz)
        //     //     for (int iy = 0; iy < BlockType::sizeY; ++iy)
        //     //     {
        //     //         const int ix = BlockType::sizeX-1;
        //     //         const Real rho = b(ix,iy,iz).alpha1rho1 + b(ix,iy,iz).alpha2rho2;
        //     //         const Real E   = b(ix,iy,iz).energy;
        //     //         const Real ru  = b(ix,iy,iz).ru;
        //     //         const Real rv  = b(ix,iy,iz).rv;
        //     //         const Real rw  = b(ix,iy,iz).rw;
        //     //         const Real a2  = b(ix,iy,iz).alpha2;
        //     //         const Real pressure = Simulation_Environment::pressure_5eq(rho, E, ru, rv, rw, a2);

        //     //         myBblock.fe[0 + 3*iy + 3*BlockType::sizeY*iz].pressure = pressure;
        //     //         myBblock.fe[1 + 3*iy + 3*BlockType::sizeY*iz].pressure = pressure;
        //     //         myBblock.fe[2 + 3*iy + 3*BlockType::sizeY*iz].pressure = pressure;
        //     //     }
        //     bc.template applyBC_1dchar_ode<0,1>(myBblock);
        // }
        ...
        //     ElementTypeBlock b_suck;
        //     const double rhoout = CloudData::rho1;
        //     const double pout = CloudData::p1;
        //     const double c2 = Simulation_Environment::GAMMA1/rhoout*(pout + Simulation_Environment::PC1);
        //     const double uout = 1.08*std::sqrt(c2);
        //     const double energy = G*pout + F*G;

        //     b_suck.alpha1rho1 = rhoout;
        //     b_suck.alpha2rho2 = 0.0;
        //     b_suck.ru = rhoout*uout;
        //     b_suck.rv = 0.0;
        //     b_suck.rw = 0.0;
        //     b_suck.energy = energy;
        //     b_suck.alpha2 = 0.0;
        //     bc.template applyBC_dirichlet<0,1>(b_suck);
        // }
...
public:
        ...
        if (info.index[0]==0)
        {
            ...
            bc.template applyBC_absorbing_tensorials_edges_corners<0,0>();
        }
        if (info.index[0]==this->NX-1)
        {
            bc.template applyBC_absorbing<0,1>();
            bc.template applyBC_absorbing_tensorials_edges_corners<0,1>();
        }
        if (info.index[1]==0)
        {
            bc.template applyBC_absorbing<1,0>();
            bc.template applyBC_absorbing_tensorials_edges_corners<1,0>();
        }
        if (info.index[1]==this->NY-1)
        {
            bc.template applyBC_absorbing<1,1>();
            bc.template applyBC_absorbing_tensorials_edges_corners<1,1>();
        }
        if (info.index[2]==0)
        {
            bc.template applyBC_absorbing<2,0>();
            bc.template applyBC_absorbing_tensorials_edges_corners<2,0>();
        }
        if (info.index[2]==this->NZ-1)
        {
            bc.template applyBC_absorbing<2,1>();
            bc.template applyBC_absorbing_tensorials_edges_corners<2,1>();
        }
    }
};
...
template<typename BlockType, template<typename X> class allocator=std::allocator>
class BlockLabSICCloud_LeftReflect_Symmetric: public BlockLab<BlockType,allocator>
{
    typedef typename BlockType::ElementType ElementTypeBlock;

public:
    BlockLabSICCloud_LeftReflect_Symmetric(): BlockLab<BlockType,allocator>(){}

    virtual inline std::string name() const { return "BlockLabSICCloud_LeftReflect_Symmetric"; }
    bool is_xperiodic() {return false;}
    bool is_yperiodic() {return false;}
    bool is_zperiodic() {return false;}
...
template<typename BlockType, template<typename X> class allocator=std::allocator>
class BlockLabSICCloud_LeftReflect_Symmetric_StressTensorial: public BlockLab<BlockType,allocator>
{
    typedef typename BlockType::ElementType ElementTypeBlock;

public:
    BlockLabSICCloud_LeftReflect_Symmetric_StressTensorial(): BlockLab<BlockType,allocator>(){}

    virtual inline std::string name() const { return "BlockLabSICCloud_LeftReflect_Symmetric_StressTensorial"; }
    bool is_xperiodic() {return false;}
    bool is_yperiodic() {return false;}
    bool is_zperiodic() {return false;}
    ...
    void _apply_bc(const BlockInfo& info, const Real t=0)
    {
        ...
        // order of if-statements matters to minimize write redundancy
        if (info.index[2]==this->NZ-1)
        {
            bc.template applyBC_absorbing<2,1>();
            bc.template applyBC_absorbing_tensorials_edges_corners<2,1>();
        ...
        if (info.index[0]==this->NX-1)
        {
            bc.template applyBC_absorbing<0,1>();
            bc.template applyBC_absorbing_tensorials_edges_corners<0,1>();
            // bc.template applyBC_dirichlet_tensorial_all<0,1>(SICCloudBCData::boundaryElement0);
        }
        if (info.index[1]==0)
        {
            bc.template applyBC_reflecting<1,0>();
            bc.template applyBC_reflecting_tensorials_edges_corners<1,0>();
        }
        if (info.index[0]==0)
        {
            bc.template applyBC_noslip_simple<0,0>();
            bc.template applyBC_noslip_simple_tensorials_edges_corners<0,0>();
        }
    }
};
