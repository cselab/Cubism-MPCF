...
#include "Diffusion_CPP_nonuniform.h"
    ...
    struct FlowStepNonuniform
    {
        ...
        inline void operator()(Lab& lab, const BlockInfo& info, Block_t& o) const
        {
            ...
            kernel.compute_nonuniform(srcfirst, Block_t::gptfloats, labSizeRow, labSizeSlice,
                    destfirst, Block_t::gptfloats, Block_t::sizeX, Block_t::sizeX*Block_t::sizeY,
                    o.coeffsWENO_x[Block_t::WENO::plus], o.coeffsWENO_x[Block_t::WENO::minus],
                    o.coeffsWENO_y[Block_t::WENO::plus], o.coeffsWENO_y[Block_t::WENO::minus],
                    o.coeffsWENO_z[Block_t::WENO::plus], o.coeffsWENO_z[Block_t::WENO::minus],
...
// #endif
        ...
        inline void operator()(Lab& lab, const BlockInfo& info, Block_t& o) const
        {
            Kernel kernel(mu1, mu2, dtinvh, info.h_gridpoint);
    ...
    struct DiffusionNonuniform
    {
        ...
        DiffusionNonuniform(const Real _dt, const Real _mu1=0, const Real _mu2=0):
#ifdef _SECONDORDER_FD_CORE_
            dt(_dt), mu1(_mu1), mu2(_mu2), stencil(-1,-1,-1,2,2,2, true, 6, 0,1,2,3,4,6)
            {
                stencil_start[0] = stencil_start[1] = stencil_start[2] = -1;
                stencil_end[0] = stencil_end[1] = stencil_end[2] = 2;
            }
#else
        dt(_dt), mu1(_mu1), mu2(_mu2), stencil(-2,-2,-2,3,3,3, true, 6, 0,1,2,3,4,6)
        {
        ...
        DiffusionNonuniform(const DiffusionNonuniform& c):
#ifdef _SECONDORDER_FD_CORE_
            dt(c.dt), mu1(c.mu1), mu2(c.mu2), stencil(-1,-1,-1,2,2,2, true, 6, 0,1,2,3,4,6)
            {
                stencil_start[0] = stencil_start[1] = stencil_start[2] = -1;
                stencil_end[0] = stencil_end[1] = stencil_end[2] = 2;
            }
#else
        dt(c.dt), mu1(c.mu1), mu2(c.mu2), stencil(-2,-2,-2,3,3,3, true, 6, 0,1,2,3,4,6)
        {
            stencil_start[0] = stencil_start[1] = stencil_start[2] = -2;
            stencil_end[0] = stencil_end[1] = stencil_end[2] = 3;
        }
        ...
        inline void operator()(Lab& lab, const BlockInfo& info, Block_t& o) const
        {
            Kernel kernel(dt, mu1, mu2);

#ifdef _SECONDORDER_FD_CORE_
            const Real * const srcfirst = &lab(-1,-1,-1).alpha1rho1;
#else
            const Real * const srcfirst = &lab(-2,-2,-2).alpha1rho1;
#endif /* _SECONDORDER_FD_CORE_ */
            const int labSizeRow = lab.template getActualSize<0>();
            const int labSizeSlice = labSizeRow*lab.template getActualSize<1>();
            Real * const destfirst =  &o.tmp[0][0][0][0];
            kernel.compute_nonuniform(srcfirst, Block_t::gptfloats, labSizeRow, labSizeSlice,
                    destfirst, Block_t::gptfloats, Block_t::sizeX, Block_t::sizeX*Block_t::sizeY,
                    o.coeffsFD_x, o.coeffsFD_y, o.coeffsFD_z,
                    &o.invh_x[0], &o.invh_y[0], &o.invh_z[0]);
        }
    };
    ...
    template < typename Kernel, typename Lab >
    struct SurfaceTensionNonuniform
    {
        StencilInfo stencil;
        Real sigma, dt;
        int stencil_start[3];
        ...
        SurfaceTensionNonuniform(const Real _dt, const Real _sigma=0.0):
#ifdef _SECONDORDER_FD_CORE_
            dt(_dt), sigma(_sigma), stencil(-1,-1,-1,2,2,2, true, 6, 0,1,2,3,4,6)
            {
                stencil_start[0] = stencil_start[1] = stencil_start[2] = -1;
                stencil_end[0] = stencil_end[1] = stencil_end[2] = 2;
            }
#else
        dt(_dt), sigma(_sigma), stencil(-2,-2,-2,3,3,3, true, 6, 0,1,2,3,4,6)
        {
            stencil_start[0] = stencil_start[1] = stencil_start[2] = -2;
            stencil_end[0] = stencil_end[1] = stencil_end[2] = 3;
        }
        ...
        SurfaceTensionNonuniform(const SurfaceTensionNonuniform& c):
#ifdef _SECONDORDER_FD_CORE_
            dt(c.dt), sigma(c.sigma), stencil(-1,-1,-1,2,2,2, true, 6, 0,1,2,3,4,6)
            {
                stencil_start[0] = stencil_start[1] = stencil_start[2] = -1;
                stencil_end[0] = stencil_end[1] = stencil_end[2] = 2;
            }
#else
        dt(c.dt), sigma(c.sigma), stencil(-2,-2,-2,3,3,3, true, 6, 0,1,2,3,4,6)
        {
            stencil_start[0] = stencil_start[1] = stencil_start[2] = -2;
            stencil_end[0] = stencil_end[1] = stencil_end[2] = 3;
        }
...
//#endif
        ...
        inline void operator()(Lab& lab, const BlockInfo& info, Block_t& o) const
        {
            Kernel kernel(dtinvh, info.h_gridpoint, sigma);
