...
#include "GridTypes.h" // should only be included here.  Include Types.h instead
...
template <> inline void FluidBlock::Write<StreamerGridPoint>(ofstream& output) const
{
    output.write((const char *)&data[0][0][0], sizeof(FluidElement)*sizeX*sizeY*sizeZ);
}

template <> inline void FluidBlock::Read<StreamerGridPoint>(ifstream& input)
{
