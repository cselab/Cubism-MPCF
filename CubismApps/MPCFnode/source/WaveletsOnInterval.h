	...
	struct WI4
	{
		...
		void sweep2D(Real data[BS][BS])
		{
			auto sweep1D = [&] {
				...
				{
					vector<Real> temp = this->forward_sweep(&data[iy][0], BS);
			...
			auto sweep1D_inv = [&] {
				for(int iy=0; iy<BS; iy++)
				{
					vector<Real> temp = this->inverse_sweep(&data[iy][0], BS);
					copy(temp.begin(), temp.end(), &data[iy][0]);
				}
			};
		...
		{			
			...
			if(bForward)
			{
				...
				for(int iz=0; iz<BS; iz++)
					for(int iy=0; iy<BS; iy++)
					{
						vector<Real> temp = this->forward_sweep(&data[iz][iy][0], BS);
			...
			else
			{
				for(int iz=0; iz<BS; iz++)
					for(int iy=0; iy<BS; iy++)
					{
						vector<Real> temp = this->inverse_sweep(&data[iz][iy][0], BS);
						copy(temp.begin(), temp.end(), &data[iz][iy][0]);
					}	
