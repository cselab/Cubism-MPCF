...
class Test_SICCloud: public virtual Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>
{
public:
    Test_SICCloud(ArgumentParser& parser) :
        Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>(parser), m_shock(NULL) { }
...
protected:
    ...
    Real m_epsilon, m_mach, m_pressureRatio, m_shockSpeed, m_R0;
    Real m_highres_time0, m_highres_time1, m_highres_dt;
    int m_highres_skip;
    bool m_bHighres_lo, m_bHighres_hi, m_bHighres_dump;
    ShockType<TElement>* m_shock;

    virtual void _setEnvironment()
    {
        Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::_setEnvironment();

        m_highres_dt = this->parser.parseRuntime("highres_dt").asDouble();
        m_highres_time0 = this->parser.parseRuntime("highres_time0").asDouble();
        m_highres_time1 = this->parser.parseRuntime("highres_time1").asDouble();
        m_highres_skip = this->parser.parseRuntime("highres_skip").asInt();
    }
    ...
    {
        const Real norm_time = this->t * Simulation_Environment::C1 / m_R0;
        if (this->isroot)
            std::cout << "--> Normalized time is " << norm_time << std::endl;
        m_bHighres_dump = false;
        m_bHighres_lo = (m_highres_dt > 0.0) && (m_highres_time0 <= norm_time);
        m_bHighres_hi = m_bHighres_lo && (m_highres_time1 == 0.0 || norm_time <= m_highres_time1);
        if (m_bHighres_lo && m_bHighres_hi)
        {
            this->dtmax = (m_highres_dt < this->dtmax) ? m_highres_dt : this->dtmax;
            if (this->dtmax <= std::numeric_limits<Real>::epsilon())
                this->dtmax = 3.0 * std::numeric_limits<Real>::epsilon();
            if (this->isroot)
                std::cout << "High-res time resolution active: dtmax = " << this->dtmax << std::endl;
            m_bHighres_dump = (this->step_id % m_highres_skip == 0);
        }
    }
...
void Test_SICCloud<TGrid,TStepper,TSlice,TSubdomain>::_ic()
{
    // set up bubbles
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SICCloud<TGrid,TStepper,TSlice,TSubdomain>::_parameter_sic_cloud()
{
    // mixture epsilon
    ...
    // high-res settings (for finer temporal resolution of velocity-jet
    // impact on distal bubble side.  Time window must be specified)
    m_highres_dt = this->parser("highres_dt").asDouble(0.0);
    m_highres_time0 = this->parser("highres_time0").asDouble(0.0);
    m_highres_time1 = this->parser("highres_time1").asDouble(0.0);
    m_highres_skip = this->parser("highres_skip").asInt(1);
    m_bHighres_lo = m_bHighres_hi = m_bHighres_dump = false;
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SICCloud<TGrid,TStepper,TSlice,TSubdomain>::_sic_cloud_simple_analysis()
{
    std::vector<BlockInfo> vInfo = this->grid->getBlocksInfo();
    ...
    for(int i=0; i<(int)vInfo.size(); i++)
    {
        ...
        TBlock& b = *(TBlock*)info.ptrBlock;
#ifdef _NONUNIFORM_BLOCK_
        const double* const hx = info.ptr_grid_spacing[0];
        const double* const hy = info.ptr_grid_spacing[1];
        const double* const hz = info.ptr_grid_spacing[2];
#endif /* _NONUNIFORM_BLOCK_ */
        for(int iz=0; iz<TBlock::sizeZ; iz++)
            for(int iy=0; iy<TBlock::sizeY; iy++)
                for(int ix=0; ix<TBlock::sizeX; ix++)
                {
                    ...
                    const double a1 = 1.0 - a2;
#ifdef _NONUNIFORM_BLOCK_
                    const double h3 = hx[ix] * hy[iy] * hz[iz];
#endif /* _NONUNIFORM_BLOCK_ */
                    rInt += rhoMix * h3;
                    uInt += b(ix,iy,iz).ru * h3;
                    vInt += b(ix,iy,iz).rv * h3;
                    wInt += b(ix,iy,iz).rw * h3;
                    eInt += b(ix,iy,iz).energy * h3;
                    vol  += a2 * h3;
                    ke   += 0.5/rhoMix * (b(ix,iy,iz).ru*b(ix,iy,iz).ru+b(ix,iy,iz).rv*b(ix,iy,iz).rv+b(ix,iy,iz).rw*b(ix,iy,iz).rw) * h3;
    ...
    fprintf(f, "%d %e %e %e %e %e %e %e %e %e %e %e %e %e\n", this->step_id, this->t, this->dt, rInt, uInt,
            vInt, wInt, eInt, vol, ke, mach_max, p_max, p_min, std::pow(0.75*vol/M_PI,1./3.));
    fclose(f);
}


template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SICCloud<TGrid,TStepper,TSlice,TSubdomain>::_moving_shock_default()
{
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SICCloud<TGrid,TStepper,TSlice,TSubdomain>::_stationary_shock_rpp()
{
    const double gamma = Simulation_Environment::GAMMA1;
    const double pc    = Simulation_Environment::PC1;
    const double G1 = 1.0 / (gamma - 1.0);
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SICCloud<TGrid,TStepper,TSlice,TSubdomain>::_stationary_shock_upp()
{
    const double gamma = Simulation_Environment::GAMMA1;
    const double pc    = Simulation_Environment::PC1;
    const double G1 = 1.0 / (gamma - 1.0);
