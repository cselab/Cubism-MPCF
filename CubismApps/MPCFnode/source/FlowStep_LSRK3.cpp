...
template<typename Kflow, typename Kdiff, typename Ksurf, typename Kupdate, typename Ksharp, typename Ksource>
struct LSRKstep
{
    template<typename T>
    ...
    LSRKstep(Grid_t& grid, const Real dt, const Real current_time, ArgumentParser& parser, const double epsilonSharpening=-1)
    {
        vector<BlockInfo> vInfo = grid.getBlocksInfo();
        ...
        timings.push_back(step(grid, vInfo, parser, A[0], B[0], dt, current_time, epsilonSharpening));
        timings.push_back(step(grid, vInfo, parser, A[1], B[1], dt, current_time + B[0]*dt, epsilonSharpening));
        timings.push_back(step(grid, vInfo, parser, A[2], B[2], dt, current_time + (B[0]+(A[1]+1.0)*B[1])*dt, epsilonSharpening));
    ...
    vector<double> step(Grid_t& grid, vector<BlockInfo>& vInfo, ArgumentParser& parser, Real a, Real b, const Real dt, const Real current_time, const double epsilonSharpening)
    {
        Timer timer;
        vector<double> res;
...
///////////////////////////////////////////////////////////////////////////////
    ...
    {
        std::cout << "Computing diffusion" << std::endl;
#ifdef _NONUNIFORM_BLOCK_
        LSRK3data::DiffusionNonuniform<Kdiff,Lab> diffusion(dt, Simulation_Environment::MU1, Simulation_Environment::MU2);
#else
...
///////////////////////////////////////////////////////////////////////////////
#ifdef _USE_HPM_
    if (LSRK3data::step_id>0)             HPM_Start("SURFACETENSION");
#endif
    if(Simulation_Environment::SIGMA>0)
    {
        std::cout << "Computing surface tension" << std::endl;
#ifdef _NONUNIFORM_BLOCK_
        LSRK3data::SurfaceTensionNonuniform<Ksurf, Lab> surfacetension(dt, Simulation_Environment::SIGMA);
#else
        LSRK3data::SurfaceTension<Ksurf, Lab> surfacetension(dt/h, Simulation_Environment::SIGMA);
#endif /* _NONUNIFORM_BLOCK_ */
        timer.start();
        processOMP<Lab>(surfacetension, grid, current_time);
        const double tsurftension = timer.stop();
    }
#ifdef _USE_HPM_
    if (LSRK3data::step_id>0)             HPM_Stop("SURFACETENSION");
#endif
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
...
#endif
    const bool source_bodyforce = parser("source_bodyforce").asBool(false);
    if(source_bodyforce)
    {
    ...
    const bool source_oneway = parser("source_oneway").asBool(false);
    if(source_oneway)
    {
        ...
        p.x0        = parser("pulse_x0").asDouble(0.5*Simulation_Environment::extents[0]);
        p.amplitude = parser("pulse_amplitude").asDouble(100.0e-5);
        p.T         = 1.0/parser("pulse_frequency").asDouble(10e3);
        p.omega     = 2.0*M_PI/p.T;
        p.sigma_t   = parser("pulse_std").asDouble(0.010/Simulation_Environment::C1);
        p.t0        = parser("pulse_t0").asDouble(3.0*p.sigma_t);
...
Real FlowStep_LSRK3::operator()(const Real max_dt, const Real current_time)
{
    ...
    cout << "sos max is " << maxSOS << ", " << "dt is "<< dt << "\n";
    ...
    if (parser("-kernels").asString("cpp")=="cpp")
        LSRKstep<Convection_CPP_HLLC_5eq_nonuniform, Diffusion_CPP_NU, SurfaceTension_CPP_NU, Update_CPP, InterfaceSharpening_CPP_cell_compact_5eq, Source_CPP>(grid, dt, current_time, parser, epsilonSharpening);
// #if defined(_QPX_) || defined(_QPXEMU_)
//     else if (parser("-kernels").asString("cpp")=="qpx")
// #if defined(_BGQ_)
//         LSRKstep<Convection_QPX_HLLC_5eq, Diffusion_CPP, Update_CPP, InterfaceSharpening_CPP_cell_compact_5eq, Source_CPP>(grid, dt, current_time, parser, epsilonSharpening);
// #else
//     LSRKstep<Convection_QPX_HLLC_5eq, Diffusion_CPP, Update_QPX, InterfaceSharpening_CPP_cell_compact_5eq, Source_CPP>(grid, dt, current_time, parser, epsilonSharpening);
// #endif
// #endif
    else
    {
    ...
    {
        if (parser("-kernels").asString("cpp")=="cpp")
            LSRKstep<Convection_CPP_HLLC_5eq, Diffusion_CPP, SurfaceTension_CPP, Update_CPP, InterfaceSharpening_CPP_cell_compact_5eq, Source_CPP>(grid, dt, current_time, parser, epsilonSharpening);
#if defined(_QPX_) || defined(_QPXEMU_)
        else if (parser("-kernels").asString("cpp")=="qpx")
#if defined(_BGQ_)
            LSRKstep<Convection_QPX_HLLC_5eq, Diffusion_CPP, SurfaceTension_CPP, Update_CPP, InterfaceSharpening_CPP_cell_compact_5eq, Source_CPP>(grid, dt, current_time, parser, epsilonSharpening);
#else
            LSRKstep<Convection_QPX_HLLC_5eq, Diffusion_CPP, SurfaceTension_CPP, Update_QPX, InterfaceSharpening_CPP_cell_compact_5eq, Source_CPP>(grid, dt, current_time, parser, epsilonSharpening);
#endif
#endif
        else
        {
    ...
    {
        if (parser("-kernels").asString("cpp")=="cpp")
            LSRKstep<Convection_CPP_HLLC_5eq, Diffusion_CPP, SurfaceTension_CPP, Update_CPP, InterfaceSharpening_CPP_5eq, Source_CPP>(grid, dt, current_time, parser, epsilonSharpening);
#if defined(_QPX_) || defined(_QPXEMU_)
        else if (parser("-kernels").asString("cpp")=="qpx")
#if defined(_BGQ_)
            LSRKstep<Convection_QPX_HLLC_5eq, Diffusion_CPP, SurfaceTension_CPP, Update_CPP, InterfaceSharpening_CPP_5eq, Source_CPP>(grid, dt, current_time, parser, epsilonSharpening);
#else
            LSRKstep<Convection_QPX_HLLC_5eq, Diffusion_CPP, SurfaceTension_CPP, Update_QPX, InterfaceSharpening_CPP_5eq, Source_CPP>(grid, dt, current_time, parser, epsilonSharpening);
#endif
#endif
        else
        {
