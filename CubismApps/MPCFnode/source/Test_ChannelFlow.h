...
class Test_ChannelFlow: public virtual Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>
{
public:
...
protected:
    ...
    typedef typename Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::TDeserializer TDeserializer;
    ...
    void _init_channel_flow() const
    {
        if (this->isroot)
        {
            ...
            if (this->parser.exist("-Re")) // laminar case
                std::cout << "Re     = " << this->Re_tau << std::endl;
            else // turbulent case
            {
                std::cout << "Re_tau = " << this->Re_tau << std::endl;
#ifdef _NONUNIFORM_BLOCK_
                const double y0 = (this->m_nonuniform->get_map_y()).cell_width(0);
#else
                const double y0 = (this->grid->getBlocksInfo())[0].h_gridpoint;
#endif /* _NONUNIFORM_BLOCK_ */
                std::cout << "y+     = " << this->Re_tau * 0.5*y0 / this->delta << std::endl;
            }
...
#ifndef NDEBUG
#ifdef _NONUNIFORM_BLOCK_
            const size_t Ncells = (this->m_nonuniform->get_map_y()).ncells();
            std::ofstream out("gridspacing.dat");
            out << "# " << (this->m_nonuniform->get_map_y()).kernel_name() << std::endl;
            double y = 0.0;
            for (int i = 0; i < Ncells; ++i)
            {
                const double h = (this->m_nonuniform->get_map_y()).cell_width(i);
                out << i+1 << '\t' << y+0.5*h << '\t' << h << std::endl;
                y += h;
            }
#endif /* _NONUNIFORM_BLOCK_ */
#endif /* NDEBUG */
...
void Test_ChannelFlow<TGrid,TStepper,TSlice,TSubdomain>::_set_forcing_laminar()
{
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_ChannelFlow<TGrid,TStepper,TSlice,TSubdomain>::_set_forcing_turbulent()
{
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_ChannelFlow<TGrid,TStepper,TSlice,TSubdomain>::_match_velocity_energy(const double umeanIC)
{
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_ChannelFlow<TGrid,TStepper,TSlice,TSubdomain>::_ic_channel_single_phase()
{
    std::vector<BlockInfo> vInfo = this->grid->getBlocksInfo();
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_ChannelFlow<TGrid,TStepper,TSlice,TSubdomain>::_ic_channel_single_phase_turbulent(const int offset)
{
    std::vector<BlockInfo> vInfo = this->grid->getBlocksInfo();
            ...
            for(int iz=0; iz<TBlock::sizeZ; iz++)
                for(int iy=0; iy<TBlock::sizeY; iy++)
                    for(int ix=0; ix<TBlock::sizeX; ix++)
                    {
                        ...
                        const double u = 1.5*u0*(1.0 - yd*yd);
                        // const double u = u0;
                        // const double u = LEE_MOSER::LM_Umean_Retau180(p[1]/delta)*u_tau;
...
namespace LEE_MOSER
{
    double LM_Umean_Retau180(double q)
    {
        const int Ndata = 96;
