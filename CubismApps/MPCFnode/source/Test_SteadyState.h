...
protected:
    ...
    // nonuniform grids
    NonUniformScheme<typename TGrid::BlockType, WENOCoefficientGenerator, FDCoefficientGenerator>* m_nonuniform;
    ...
    // output generation
    OutputProcessing<TGrid,TSlice,TSubdomain> * dumper;
    ...
    {
        std::ios_base::fmtflags f( _of.flags() );
    ...
    virtual void _setup_ic(TDeserializer _deserializer)
    {
        ...
        {
            if ((this->parser("sim").asString() == "steady") && this->parser("ic_trigonometric").asBool(false))
                this->_ic_trigonometric();
            else
                this->_ic();
        }
    }
    ...
    virtual void _init()
    {
        ...
        if (parser("dumplistall").asBool(false))
            dumper->show_processor_entries();
    }
    virtual void _ic();
    void _ic_trigonometric();
...
void Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::_deserialize(TDeserializer _kern)
{
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::_serialize(TSerializer _kern)
{
    const std::string path = parser("-fpath").asString(".");
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::_infoBoard() const
{
    // save state
    ...
    std::cout << "----------------------------------------------------------------------" << std::endl;
    {
        ...
        std::cout << std::left << "Non-uniform blocks?"  << ": YES" << std::endl;
        std::cout.width(50); std::cout.fill('.');
#ifdef _SECONDORDER_FD_CORE_
        std::cout << std::left << "  Using 2nd-order finite differences on core layer" << std::endl;
#else
        std::cout << std::left << "  Using 4th-order finite differences on core layer" << std::endl;
#endif /* _SECONDORDER_FD_CORE_ */
#else
        std::cout << std::left << "Non-uniform blocks?"  << ": NO" << std::endl;
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::_setup_parameter()
{
    parser.mute();
    ...
    if (parser("restart_format").asString("zbin") == "zbin") // we prioritize zbin
        _serializationKernel = &DumpZBin<StreamerSerialization, TGrid>;
#ifdef _USE_HDF_
    else if (parser("restart_format").asString("zbin") == "h5")
        _serializationKernel = &DumpHDF5<StreamerSerialization, DumpReal, TGrid>;
#endif
    else
    {
        if (isroot)
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::_ic_trigonometric()
{
    ...
    const double a1r1 = 1.0;
    const double a2r2 = 0.0;
    const double u    = parser("u").asDouble(1.0);
    const double v    = parser("v").asDouble(2.0);
    const double w    = parser("w").asDouble(3.0);
    const double a2   = 0.0;
    ...
    const double Fx = 2.0*M_PI/Simulation_Environment::extents[0];
    const double Fy = 2.0*M_PI/Simulation_Environment::extents[1];
    const double Fz = 2.0*M_PI/Simulation_Environment::extents[2];
        ...
        for(int iz=0; iz<TBlock::sizeZ; iz++)
            for(int iy=0; iy<TBlock::sizeY; iy++)
                for(int ix=0; ix<TBlock::sizeX; ix++)
                {
                    double p[3];
                    info.pos(p, ix, iy, iz);
                    const double uu = u * std::sin( p[0] * Fx ) * std::cos( p[1] * Fy ) * std::cos( p[2] * Fz );
                    const double vv = v * std::cos( p[0] * Fx ) * std::sin( p[1] * Fy ) * std::cos( p[2] * Fz );
                    const double ww = w * std::cos( p[0] * Fx ) * std::cos( p[1] * Fy ) * std::sin( p[2] * Fz );
                    b(ix, iy, iz).alpha1rho1= a1r1;
                    b(ix, iy, iz).alpha2rho2= a2r2;
                    b(ix, iy, iz).ru = r*uu;
                    b(ix, iy, iz).rv = r*vv;
                    b(ix, iy, iz).rw = r*ww;
                    b(ix, iy, iz).energy = Gm*1.0 + Pm + 0.5*r*(uu*uu + vv*vv + ww*ww);
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::_ic()
{
    const double G1 = 1.0/(Simulation_Environment::GAMMA1-1);
    const double G2 = 1.0/(Simulation_Environment::GAMMA2-1);
    const double F1 = Simulation_Environment::GAMMA1*Simulation_Environment::PC1;
...
#pragma omp parallel for
    for(int i=0; i<(int)vInfo.size(); i++)
    {
        ...
        size_t k = i * TBlock::sizeX * TBlock::sizeY * TBlock::sizeZ;
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::run()
{
    _init();
    ...
    while (true)
    {
        ...
        // (2)
        dtmax = (*dumper)(step_id, t, NSTEPS, TEND, profiler, true, bRESTART);
...
template <typename TGrid, typename TStepper, template <typename> class TSlice, template <typename> class TSubdomain>
void Test_SteadyState<TGrid,TStepper,TSlice,TSubdomain>::setup()
{
    _setup_parameter();
    ...
    m_nonuniform = new NonUniformScheme<typename TGrid::BlockType, WENOCoefficientGenerator, FDCoefficientGenerator>(0.0,eX,0.0,eY,0.0,eZ, BPDX, BPDY, BPDZ);
    ...
    dumper = new OutputProcessing<TGrid,TSlice,TSubdomain>(parser, *grid, isroot);
    assert(dumper != NULL);
    ...
    if (exist_restart("zbin", path)) // we prioritize zbin (MPI topology must be the same as at serialization time)
        _deserializer = &ReadZBin<StreamerSerialization, TGrid>;
#ifdef _USE_HDF_
    else if (exist_restart("h5", path))
        _deserializer = &ReadHDF5<StreamerSerialization, DumpReal, TGrid>;
#endif
